...
use App\Models\RevenueToolkit;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Validator;

class RevenueToolkitController extends Controller
{
    public function index()
    {
        $user = Auth::user();
        $toolkits = RevenueToolkit::accessible($user->user_id)
        ...
        return view('pages.revenue-logsheet', compact('toolkits', 'icons', 'iconsPath'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->filled('toolkit_id')) {
            $toolkit = RevenueToolkit::findOrFail($request->input('toolkit_id'));
        ...
        RevenueToolkit::create($attributes);
