...
use App\Models\BooksToolkit;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Validator;

class BooksToolkitController extends Controller
{
    public function index()
    {
        $user = Auth::user();
        $toolkits = BooksToolkit::accessible($user->user_id)
        ...
        return view('pages.books-of-accounts', compact('toolkits', 'icons', 'iconsPath'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->filled('toolkit_id')) {
            $toolkit = BooksToolkit::findOrFail($request->input('toolkit_id'));
        ...
        BooksToolkit::create($attributes);
