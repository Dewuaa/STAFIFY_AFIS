...
use App\Models\InventoryToolkit;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Validator;

class InventoryToolkitController extends Controller
{
    public function index()
    {
        $user = Auth::user();
        $toolkits = InventoryToolkit::accessible($user->user_id)
        ...
        return view('pages.inventory-logsheet', compact('toolkits', 'icons', 'iconsPath'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->filled('toolkit_id')) {
            $toolkit = InventoryToolkit::findOrFail($request->input('toolkit_id'));
        ...
        InventoryToolkit::create($attributes);
