...
use App\Models\JournalToolkit;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Validator;

class JournalToolkitController extends Controller
{
    public function index()
    {
        $user = Auth::user();
        $toolkits = JournalToolkit::accessible($user->user_id)
        ...
        return view('pages.journal-logsheet', compact('toolkits', 'icons', 'iconsPath'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->filled('toolkit_id')) {
            $toolkit = JournalToolkit::findOrFail($request->input('toolkit_id'));
        ...
        JournalToolkit::create($attributes);
