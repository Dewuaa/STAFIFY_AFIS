...
@section('title', 'Journal Toolkit')
@section('description', 'Manage Journal toolkit resources and forms.')
    ...
    <div id="addToolkitModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
        <div class="relative min-h-screen px-4 text-center">
            ...
            <div class="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl relative">
                ...
                <form method="POST" action="{{ route('journal-toolkit.store') }}" class="mt-4">
                    @csrf
...
function openAddToolkitModal(toolkit = null) {
    ...
    if (toolkit) {
        ...
        if (iconOption) { selectIcon(iconOption); }
        ...
        if (defaultIcon) { selectIcon(defaultIcon); }
        updateToolkitTypeFields();
    }
...
<style>
/* Styles identical to other toolkit pages (copied for consistency) */
...
.square-card { width: 80px !important; height: 80px !important; min-width: 80px !important; min-height: 80px !important; max-width: 80px !important; max-height: 80px !important; padding: 0 !important; margin: 0 auto !important; display: flex !important; align-items: center !important; justify-content: center !important; }
